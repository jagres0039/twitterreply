...
async function loadSettings() {
  const result = await chrome.storage.sync.get([
    'groqApiKey', 'autoDetect', 'showButton', 'includeEmojis', 'universalMode'
  ]);
  ...
  // responseStyle removed — replaced by reply tone buttons
...
function setupEventListeners() {
  ...
  // Blacklist
  document.getElementById('blacklistAdd')?.addEventListener('click', addBlacklistWord);
  document.getElementById('blacklistInput')?.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') addBlacklistWord();
  });
  document.getElementById('exportHistory')?.addEventListener('click', exportHistory);
  document.getElementById('exportSaved')?.addEventListener('click', exportSavedReplies);
...
// Style preview removed — replaced by Reply Tone buttons
...
const TONE_DESCRIPTIONS = {
  auto:     'AI pilih tone yang paling sesuai konteks',
  serious:  'Balas dengan argumen serius & substantif',
  funny:    'Bercanda, witty, bikin orang senyum',
  question: 'Nanya balik — bikin mereka mikir lebih',
  roast:    'Sarkastis ringan, gak kasar',
  support:  'Validasi, encourage, empati',
};
...
function setActiveTone(tone) {
  ...
  const desc = document.getElementById('toneDesc');
  if (desc) desc.textContent = TONE_DESCRIPTIONS[tone] || '';
}
...
function renderBlacklist(words) {
  _blacklistWords = words || [];
  const container = document.getElementById('blacklistTags');
  if (!container) return;
  container.innerHTML = '';
  _blacklistWords.forEach((word, idx) => {
    const tag = document.createElement('span');
    tag.className = 'blacklist-tag';
    tag.innerHTML = `${escapeHtml(word)}<button title="Hapus" data-idx="${idx}">×</button>`;
    tag.querySelector('button').addEventListener('click', () => removeBlacklistWord(idx));
...
function addBlacklistWord() {
  ...
  if (!word) return;
  if (_blacklistWords.includes(word)) {
    showStatus('⚠️ Kata sudah ada di list', 'error');
    return;
  }
  _blacklistWords.push(word);
  chrome.storage.sync.set({ blacklistWords: _blacklistWords }, () => {
    renderBlacklist(_blacklistWords);
    input.value = '';
    showStatus(`🚫 "${word}" ditambah ke blacklist`, 'success');
  });
}

function removeBlacklistWord(idx) {
  _blacklistWords.splice(idx, 1);
  chrome.storage.sync.set({ blacklistWords: _blacklistWords }, () => {
    renderBlacklist(_blacklistWords);
  });
}
...
function loadSavedReplies(callback) {
  chrome.storage.local.get(['savedReplies'], (data) => {
    callback(data.savedReplies || []);
  });
}

function saveReply(text, context) {
  loadSavedReplies((replies) => {
    const entry = {
      id: Date.now(),
      text,
      context: context || '',
      savedAt: new Date().toLocaleDateString('id-ID', { day:'2-digit', month:'short', year:'numeric' })
    };
    const updated = [entry, ...replies].slice(0, MAX_SAVED);
    chrome.storage.local.set({ savedReplies: updated }, () => {
      renderSavedReplies(updated);
      showStatus('💾 Reply tersimpan!', 'success');
    });
  });
}
...
function renderSavedReplies(replies) {
  ...
  if (!replies || replies.length === 0) {
    container.innerHTML = '<div class="saved-empty">Belum ada reply tersimpan</div>';
    return;
  }
  container.innerHTML = '';
  replies.forEach(item => {
    ...
    el.innerHTML = `
      <div class="saved-item-meta">
        <span class="saved-item-topic">${escapeHtml(item.context || 'Reply')}</span>
        <span class="saved-item-date">${item.savedAt}</span>
      </div>
      <div class="saved-item-text">${escapeHtml(item.text)}</div>
      <div class="saved-item-actions">
        <button class="saved-copy-btn">📋 COPY</button>
        <button class="saved-del-btn">🗑️ HAPUS</button>
      </div>`;
    el.querySelector('.saved-copy-btn').addEventListener('click', () => copyRaw(item.text));
    el.querySelector('.saved-del-btn').addEventListener('click', () => deleteSavedReply(item.id));
    container.appendChild(el);
  });
}

// Auto-render saved replies on popup open
loadSavedReplies(renderSavedReplies);

// ===== REPLY HISTORY =====
function loadHistory(callback) {
  chrome.storage.local.get(['replyHistory'], (data) => callback(data.replyHistory || []));
}
...
function renderHistory(items) {
  ...
  if (!items || items.length === 0) {
    container.innerHTML = '<div class="history-empty">Belum ada history — reply yang lo copy akan otomatis masuk sini</div>';
    return;
  }
  container.innerHTML = '';
  items.forEach(item => {
    ...
    el.innerHTML = `
      <div class="history-meta">
        <span class="history-tweet-ctx">${escapeHtml(item.tweetText || '—')}</span>
        <span class="history-time">${item.loggedAt}</span>
      </div>
    ...
    el.querySelector('.history-copy-btn').addEventListener('click', () => copyRaw(item.text));
    el.querySelector('.history-save-btn').addEventListener('click', () => {
      saveReply(item.text, item.tweetText);
    });
    el.querySelector('.history-del-btn').addEventListener('click', () => {
      loadHistory(hist => {
        const updated = hist.filter(h => h.id !== item.id);
        chrome.storage.local.set({ replyHistory: updated }, () => renderHistory(updated));
      });
    });
    container.appendChild(el);
  });
...
function clearHistory() {
  ...
  chrome.storage.local.set({ replyHistory: [] }, () => renderHistory([]));
}

function exportHistory() {
  loadHistory(function(items) {
    if (!items.length) { showStatus('Export gagal: history kosong', 'error'); return; }
    var NL = '\n';
    var csvRows = ['"No","Reply","Context","Time"'];
    items.forEach(function(item, i) {
      var esc = function(s) { return '"' + (s||'').replace(/"/g,'""') + '"'; };
      csvRows.push([i+1, esc(item.text), esc(item.tweetText), esc(item.loggedAt)].join(','));
    });
    var csvContent = csvRows.join(NL);
    var txtContent = items.map(function(item, i) {
      return '[' + (i+1) + '] ' + (item.loggedAt||'') + NL +
             'Context: ' + (item.tweetText||'-') + NL +
             'Reply: ' + (item.text||'') + NL;
    }).join(NL + '---' + NL + NL);
    var useCsv = confirm('OK = Export CSV  /  Batal = Export TXT');
    var blob = new Blob([useCsv ? csvContent : txtContent], { type: 'text/plain' });
    var url = URL.createObjectURL(blob);
    var a = document.createElement('a');
    a.href = url;
    a.download = 'reply-history-' + new Date().toISOString().slice(0,10) + (useCsv ? '.csv' : '.txt');
    a.click();
    URL.revokeObjectURL(url);
    showStatus('Exported ' + items.length + ' replies', 'success');
  });
}

// Also add export for saved replies
function exportSavedReplies() {
  loadSavedReplies(function(items) {
    if (!items.length) { showStatus('Saved replies kosong', 'error'); return; }
    var NL = '\n';
    var txtContent = items.map(function(item, i) {
      return '[' + (i+1) + '] ' + (item.savedAt||'') + NL +
             'Context: ' + (item.context||'-') + NL +
             'Reply: ' + (item.text||'') + NL;
    }).join(NL + '---' + NL + NL);
    ...
    var a = document.createElement('a');
    a.href = url;
    a.download = 'saved-replies-' + new Date().toISOString().slice(0,10) + '.txt';
    a.click();
    URL.revokeObjectURL(url);
    showStatus('Exported ' + items.length + ' saved replies', 'success');
  });
}

// Auto-load history on popup open
...
function savePersona() {
  ...
  chrome.storage.sync.set({ userPersona: text }, () => {
    if (text) {
      showStatus('✅ Persona disimpan! AI akan pakai gaya ini.', 'success');
      if (badge) badge.style.display = 'inline-block';
    } else {
      showStatus('🗑️ Persona dihapus — balik ke default.', 'warning');
