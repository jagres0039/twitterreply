...
function buildSystemPrompt(language, persona = '') {
  // Few-shot examples per language — show AI what real Twitter looks like
  const fewShots = {
    indonesian: `
CONTOH REPLY MANUSIA DI TWITTER INDONESIA (pelajari gaya, JANGAN copy):

Tweet: "baru sadar kalau gue udah 3 tahun di kerjaan yang sama tanpa naik gaji"
Reply A: "3 tahun bro... itu udah cukup buat nanya baik-baik atau keluar"
Reply B: "ini tanda buat mulai kirim CV lagi"
Reply C: "setidaknya cv lo ada isinya sekarang lol"

Tweet: "kenapa ya orang indo susah banget minta maaf duluan"
Reply A: "ego + takut keliatan lemah, combo mematikan"
Reply B: "dibesarkan buat merasa minta maaf = kalah"
Reply C: "karena minta maaf itu susah kalau dari kecil gak pernah diajarin"

Tweet: "katanya hustle culture toxic tapi kenapa yg sukses rata2 emang kerja keras"
Reply A: "kerja keras iya, tapi hustle culture itu yg glorifikasi burnout-nya"
Reply B: "survivorship bias bro, yg burnout dan gagal gak ada yg share"
Reply C: "beda antara kerja keras dan kerja tanpa tidur selama 3 tahun"

Tweet: "abis makan malam sendirian di restoran buat pertama kali, rasanya... bebas?"
Reply A: "welcome to the club, ini salah satu privilege hidup sendiri"
Reply B: "meja buat 1 itu underrated jujur"`.trim(),

    english: `
EXAMPLES OF REAL TWITTER REPLIES (study the vibe, DON'T copy):

Tweet: "just realized I've been at the same job for 3 years with no raise"
Reply A: "3 years is more than enough notice to start sending applications"
Reply B: "your loyalty is showing and it's not being reciprocated"
Reply C: "that's a resignation letter waiting to be written lol"

Tweet: "why is it so hard for people to just admit they were wrong"
Reply A: "ego + the weird idea that being wrong means being weak"
Reply B: "nobody taught them that changing your mind is actually smart"
Reply C: "because they'd rather reframe than reflect"

Tweet: "hustle culture is toxic they say, but somehow the successful people always worked hard"
Reply A: "working hard ≠ hustle culture. hustle culture glorifies the burnout part"
Reply B: "survivorship bias. the ones who burned out and failed aren't posting"
Reply C: "there's a difference between discipline and running yourself into the ground"`.trim(),

    japanese: `
日本語Twitterの実際の返信例（スタイルを学ぶ、コピーしない）:

ツイート: "3年同じ会社で昇給なしってやっと気づいた"
返信A: "3年あれば十分交渉か転職考える理由になる"
返信B: "その忠誠心、報われてないじゃん"
返信C: "履歴書更新するサインかもね"

ツイート: "なんで人って素直に�謝れないんだろ"
返信A: "プライドと弱く見られたくない気持ちのコンボ"
返信B: "子どものころ謝ることを教えてもらえなかったから"
返信C: "謝ったら負けって思ってる人多い気がする"`.trim()
  };

  const basePersona = {
    indonesian: `Kamu orang Indonesia asli, 20-an, aktif di Twitter/X setiap hari.
Nulis kayak lagi chat — singkat, spontan, punya opini sendiri.
PANTANGAN KERAS:
- Jangan mulai dengan "Wah", "Haha", "Oke", "Tentu", "Sip", "Memang benar bahwa", "Hal ini"
- Jangan pakai "sebagai", "adapun", "terlebih lagi", "dengan demikian"
- Jangan emoji di awal. Max 1 emoji total.
- Jangan kedengeran kayak artikel, chatbot, atau press release
- REPLY PENDEK: 1-2 kalimat. Kalau bisa 1, jangan 2.
Output: HANYA teks reply. Langsung. Tanpa label.`,

    english: `You're a real person, mid-20s, chronically online on Twitter/X.
You write like you text — short, direct, with an actual personality.
HARD RULES:
- Never start with "Great", "Sure", "Of course", "Absolutely", "Wow", "Indeed", "Certainly"
- Never use "Furthermore", "Moreover", "Additionally", "In conclusion", "It is worth noting"
- No emoji at start. Max 1 emoji total.
- Don't sound like a blog post, chatbot, or LinkedIn post
- SHORT: 1-2 sentences. If 1 works, use 1.
Output: ONLY reply text. Direct. No labels.`,

    japanese: `あなたは20代の日本人。毎日Twitter/Xを使っている。
LINEで友達に送るように書く — 短く、率直、自分の意見あり。
禁止:
- 「なるほど」「確かに」「素晴らしい」「おっしゃる通り」で始めない
- 「さらに」「したがって」「つまり」「まとめると」使わない
- 文頭絵文字禁止。全体で最大1個。
- ブログ・チャットボット風禁止
- 短く: 1〜2文。1文で済むなら1文。
出力: 返信テキストのみ。ラベルなし。`
...
function buildUserPrompt(tweetText, language, topic, sentiment, enrichedContext = '', replyTone = 'auto') {
...
CARA REPLY:
- BERTANYA jawab langsung, 1 kalimat cukup
- OPINI/CERITA share sudut pandang lo, singkat
- Ada gambar/link komentari kontennya
- Jangan generik tanpa isi
- Ini Twitter, BUKAN forum atau blog
${enrichedContext}
...
HOW TO REPLY:
- QUESTION: answer directly, 1 sentence
- OPINION/STORY: your take, keep it tight
- Image/link: comment on the actual content
- No filler without substance
- NOT a blog post — this is Twitter
${enrichedContext}
...
async function generateResponse(tweetContext, userStyle = 'casual', includeEmojis = true, forceTone = null) {
  ...
  const userPrompt   = buildUserPrompt(tweetText, language, topic, sentimentResult.sentiment, enrichedContext, replyTone || 'auto');
  const lengthGuide  = calcReplyLength(tweetText);
  if (!groqApiKey?.startsWith('gsk_')) throw new Error('Invalid API key');
  ...
  const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      temperature: language === 'indonesian' ? 0.78 : language === 'japanese' ? 0.70 : 0.72,
      top_p: 0.92,
  ...
  // Apply language-specific humanizer
  if (language === 'english') reply = humanizeText(reply);
  else if (language === 'indonesian') reply = humanizeIndonesian(reply);
...
async function generateMultipleResponses(tweetContext, userStyle = 'casual', includeEmojis = true, numResponses = 3, replyTone = 'auto') {
  ...
  const userResp = await generateResponse(tweetContext, userStyle, includeEmojis, replyTone);
  ...
  for (let i = 1; i < numResponses; i++) {
    ...
    const resp = await generateResponse(tweetContext, randStyle, includeEmojis, replyTone);
...
function humanizeIndonesian(text) {
  let r = text.trim();
  ...
  // Natural Indonesian shortenings
  r = r
    .replace(/\btidak\b/g, () => Math.random() > 0.4 ? 'gak' : 'tidak')
    .replace(/\bsudah\b/g,  () => Math.random() > 0.4 ? 'udah' : 'sudah')
    .replace(/\bkalau\b/g,  () => Math.random() > 0.5 ? 'kalo' : 'kalau')
    .replace(/\btapi\b/g,   () => Math.random() > 0.5 ? 'tp' : 'tapi')
    .replace(/\byang\b/g,   () => Math.random() > 0.6 ? 'yg' : 'yang')
    .replace(/\bsangat\b/g, () => Math.random() > 0.5 ? 'banget' : 'sangat')
    .replace(/\bseperti\b/g,() => Math.random() > 0.5 ? 'kayak' : 'seperti')
    .replace(/\bkarena\b/g, () => Math.random() > 0.6 ? 'karena' : 'karna')
    .replace(/\buntuk\b/g,  () => Math.random() > 0.6 ? 'buat' : 'untuk');

  // Occasionally lowercase first letter (30%)
  if (Math.random() > 0.7) r = r.charAt(0).toLowerCase() + r.slice(1);

  // Drop trailing period on short casual replies
  r = r.replace(/\s+/g, ' ').trim();
  if (r.length < 150 && r.endsWith('.')) r = r.slice(0, -1);
...
// ===== HUMAN-LIKE POST-PROCESSOR (UPGRADED) =====
function simulateHumanTyping(text, language = 'indonesian') {
  if (!text) return text;

  // Run language-specific humanizer first
  let r = language === 'indonesian' ? humanizeIndonesian(text)
        : language === 'english'    ? humanizeText(text)
        : text;

  // Universal: randomly add natural filler words (20% chance each)
  if (language === 'indonesian') {
    const fillers = ['sih', 'aja', 'deh', 'juga', 'kayaknya'];
    if (Math.random() > 0.75) {
      // Insert filler before first period/end
      const filler = fillers[Math.floor(Math.random() * fillers.length)];
      r = r.replace(/([^.!?])$/, `$1 ${filler}`);
    }
  }
  if (language === 'english') {
    const fillers = ['ngl', 'tbh', 'lol', 'honestly', 'fr'];
    if (Math.random() > 0.8) {
      const filler = fillers[Math.floor(Math.random() * fillers.length)];
      // 50/50: prepend or append
      r = Math.random() > 0.5 ? `${filler} ${r}` : `${r} ${filler}`;
    }
  }

  // 25% chance: drop trailing period
  if (Math.random() > 0.75 && r.endsWith('.')) r = r.slice(0, -1);

  // 15% chance: full lowercase (very casual vibe)
  if (Math.random() > 0.85) r = r.charAt(0).toLowerCase() + r.slice(1);
