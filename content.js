...
class TwitterAIAssistant {
  ...
  async sendMultiResponseRequest() {
    await this.loadSettings();
    // Read replyTone from storage so tone buttons actually work
    const { replyTone } = await new Promise(res =>
  ...
  logToHistory(text, tweetText) {
    chrome.storage.local.get(['replyHistory'], (data) => {
      ...
      const entry = {
        ...
        loggedAt: new Date().toLocaleString('id-ID', { day:'2-digit', month:'short', hour:'2-digit', minute:'2-digit' })
      };
      const updated = [entry, ...history].slice(0, 30); // keep last 30
      chrome.storage.local.set({ replyHistory: updated });
    });
  }
  ...
  showResponseSelector(responses, sentiment) {
            ...
            <div style="background: rgba(255,255,255,0.03); border: 1px solid rgba(64,224,255,0.25); border-radius: 12px; 
              ...
              <div style="display: flex; gap: 10px; justify-content: flex-end;">
                ...
                <button class="save-reply-btn"
                  data-text="${resp.text.replace(/"/g, '&quot;').replace(/'/g, '&apos;')}"
                  data-context="${(sentiment.tweetText||'reply').substring(0,60).replace(/"/g,'')}"
                  style="padding:8px 14px;border-radius:8px;border:1px solid rgba(255,210,0,0.4);background:rgba(255,210,0,0.1);color:#ffd200;font-family:'Orbitron',sans-serif;font-size:11px;font-weight:700;cursor:pointer;">
                  💾 SAVE
                </button>
              </div>
            </div>
    ...
    style.textContent = `
      ...
      .copy-btn:hover, .preview-btn:hover, .use-random-btn:hover, .close-modal-btn:hover, .regenerate-btn:hover {
    ...
    // Copy button handler + auto-log to history
    modal.querySelectorAll('.copy-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        const text = btn.getAttribute('data-text');
        // Auto-log to reply history
        this.logToHistory(text, sentiment.tweetText || '');
    ...
    // Save reply button handler
    modal.querySelectorAll('.save-reply-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        const text = btn.getAttribute('data-text');
        ...
        chrome.storage.local.get(['savedReplies'], (data) => {
          ...
          const updated = [entry, ...replies].slice(0, 20);
          chrome.storage.local.set({ savedReplies: updated }, () => {
            this.showSuccess('💾 Reply tersimpan di popup!');
    ...
    // Regenerate button
    const regenBtn = modal.querySelector('.regenerate-btn');
    if (regenBtn) {
      regenBtn.addEventListener('click', async (e) => {
        ...
        try {
          const result = await this.sendMultiResponseRequest();
          closeModal();
          this.showResponseSelector(result.responses, result.sentiment);
        } catch (err) {
          regenBtn.textContent = '❌ Failed';
          setTimeout(() => { regenBtn.textContent = '🔄 REGENERATE'; regenBtn.disabled = false; }, 2000);
        }
      });
    }
